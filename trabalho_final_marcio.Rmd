...
<img width="80%" src="robustez_low.png"/>

Não parece ser bimodal. Parece ter um modo, somente.
...
```
info1 <- info[-c(133, 139, 140,141,145),] %>% select_("Species", "Interactions", "Connectance")
```
...
```
dados <- cbind("Species" = info[-c(133, 139, 140,141,145),] %>% select_("Species"), "Interactions" = info[-c(133, 139, 140,141,145),] %>% select_("Interactions") , "Connectance" = info[-c(133, 139, 140,141,145),] %>% select_("Connectance") ,robustez_low,MAT, MAP, CV, TS, PDQ)
```


Mapas da robustez em função das variáveis ambientais:
...
Salvando os gráficos com o uso do pacote ```patchwork```:
...
Olhe como a robustez varia em um gradiente de condições ambientais:

<img width="90%" src="robus_vs_envir.png"/>


Agora, vamos ver com outras métricas e com gráficos mais informativos.
...
<img weight="90%" src="métricas_vs_envir.png"/>
